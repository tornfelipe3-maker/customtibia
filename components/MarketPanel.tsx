...
import { Coins, Search, ShoppingBag, Tag, History, Trash2, ArrowUpRight, Sparkles, AlertCircle, RefreshCw } from 'lucide-react';
...
export const MarketPanel: React.FC<MarketPanelProps> = ({ player, userId, onBuyMarket, onListMarket, onCancelMarket }) => {
    ...
    return (
        <div className="flex flex-col h-full bg-[#1a1a1a] text-gray-200 overflow-hidden">
                                    ...
                                    <div key={listing.id} className={`p-3 rounded border flex items-center justify-between group hover:bg-black/40 transition-all ${getRarityClass(listing.item_data.rarity)}`}>
                                        ...
                                        <div className="flex flex-col items-end gap-2">
                                            ...
                                            <button 
                                                ...
                                                `}
                                            >
                                                {listing.seller_id === userId ? 'Seu Item' : 'Comprar'}
                                            </button>
                                        </div>
                                    </div>
                                ))}
                        ...
                        <div className="grid grid-cols-5 sm:grid-cols-8 gap-3">
                            {player.uniqueInventory?.map((item) => (
                                <button
                                    ...
                                    onMouseLeave={(e) => handleHover(null, e)}
                                    onClick={() => {
                                        const price = prompt("Preço em Tibia Coins:", "25");
                                        if (price && parseInt(price) > 0) onListMarket(item, parseInt(price));
                                    }}
                                    className={`aspect-square rounded border-2 flex items-center justify-center relative hover:scale-105 transition-all shadow-md ${getRarityClass(item.rarity)}`}
                                >
                    ...
                    <div className="max-w-4xl mx-auto space-y-3">
                        {listings.filter(l => l.seller_id === userId).map(listing => (
                            <div key={listing.id} className="p-4 bg-[#222] border border-[#333] rounded flex items-center justify-between">
                                ...
                                <button 
                                    onClick={() => onCancelMarket(listing)}
                                    className="p-2 text-red-500 hover:bg-red-950/30 rounded border border-transparent hover:border-red-900 transition-all flex items-center gap-2 text-xs font-bold"
                                >
