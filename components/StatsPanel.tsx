...
import { Activity, X, TrendingUp, Zap, Swords, Coins, Shield, Sparkles, Ghost, Flame, Skull, Hammer, Crosshair } from 'lucide-react';
...
export const StatsPanel: React.FC<StatsPanelProps> = ({ player, isOpen, onClose }) => {
  ...
  const StatRow = ({ label, value, icon, color = "text-gray-300", subValue = "" }: any) => (
      <div className="flex justify-between items-center py-1 border-b border-[#333] last:border-0 group hover:bg-white/5 px-1 rounded-sm transition-colors">
          <div className="flex items-center gap-2">
              <span className={`${color} opacity-70 group-hover:opacity-100 transition-opacity`}>{icon}</span>
              <span className="text-[10px] font-bold text-gray-400 group-hover:text-gray-200">{label}</span>
          </div>
    ...
    >
        ...
        <div className="p-3 space-y-4 custom-scrollbar max-h-[70vh] overflow-y-auto bg-gradient-to-b from-[#2d2d2d] to-[#1a1a1a]">
            ...
            <div>
                ...
                <StatRow label="XP Stage" value={`${xpStage}x`} icon={<TrendingUp size={12}/>} color="text-green-400" subValue={`Current Lvl: ${player.level}`}/>
                <StatRow label="Total XP Bonus" value={`+${ascXp + eqXp}%`} icon={<Zap size={12}/>} color="text-blue-400" subValue={`Eq: ${eqXp}% | Asc: ${ascXp}%`}/>
